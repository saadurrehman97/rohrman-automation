...
# Abbreviations that appear in a Tekion account NAME and stand for the phrase
# an invoice prints. This is the whole vocabulary needed to tell which figure
# belongs to which account, because both sides already say what they are:
#
#   account 2250  "OTH DIST REC (DAP,MCB & WFR)"
#   invoice       "WHOLESALE FINANCE RESERVE OF $368.00"
#
# WFR expands to that phrase, so 368.00 is 2250's, and an OCR reading that gave
# it to 2245 can be corrected rather than merely doubted.
_NAME_EXPANSIONS = {
    "wfr": "wholesalefinancereserve",
    "ppo": "pporeserve",
    "dap": "dealeradvertisingprogram",
    "mcb": "marketingcooperativebonus",
    "hb": "holdback",
    "doc": "documentaryfee",
    "tda": "toyotadealeradvertising",
}


def _name_tokens(account_name: Any) -> set[str]:
    """What an account's name says it is, abbreviations expanded."""
    flat = _normalise(account_name)
    tokens = {flat} if flat else set()
    for short, long in _NAME_EXPANSIONS.items():
        if short in re.split(r"[^a-z0-9]+", str(account_name or "").lower()):
            tokens.add(long)
    return tokens


def _label_matches_account(label: Any, account_name: Any) -> bool:
    """Does OCR's label for an amount describe THIS account?"""
    flat = _normalise(label)
    if not flat:
        return False
    for token in _name_tokens(account_name):
        if not token:
            continue
        if token in flat or flat in token:
            return True
    return False
...
def repair_by_label(
    ...
    """Move an amount to the account its own label describes.

    OCR pairs a handwritten account with a figure by following an arrow, and on
    a page where two arrows sit above the same sentence it can cross them. The
    Oakbrook Toyota invoice is exactly that: 392.00 labelled "PPO RESERVE" went
    to 2250, and 368.00 labelled "WHOLESALE FINANCE RESERVE" went to 2245 --
    while 2250 is the account whose name ends in WFR.

    Geometry is the weak evidence here and meaning is the strong one, so the
    labels are checked against the account names and a crossed pair is swapped
    back. Only amounts whose label positively matches a DIFFERENT written
    account are moved; anything ambiguous is left exactly as OCR had it.

    Returns the corrected map and a note for each change made.
    """
    if not labels or not chart:
        return annotations, []

    # The accounts this template actually has, by number.
    names: dict[str, str] = {}
    for p in postings:
        number = gl_number_of(p.get("glAccountId"), chart)
        entry = chart.get(str(p.get("glAccountId") or "")) or {}
        if number:
            names[number] = str(entry.get("account_name") or "")

    corrected = dict(annotations)
    notes: list[str] = []

    for account, label in labels.items():
        if account not in annotations:
            continue
        # Already on the right account: nothing to do.
        if _label_matches_account(label, names.get(account, "")):
            continue
        # Does this label describe some OTHER account that was also written?
        for other in annotations:
            if other == account or other not in names:
                continue
            if _label_matches_account(label, names[other]):
                corrected[account], corrected[other] = (
                    annotations[other],
                    annotations[account],
                )
                notes.append(
                    f"{label!r} names {other} ({names[other]}), not {account} -- "
                    f"swapped {annotations[account]:,.2f} and {annotations[other]:,.2f}"
                )
                break
